...
| Punctuation candidates | Type `[` or `]`, then press ↓ | The candidate window opens with the current form first and offers only that key's related half-width/full-width/Chinese forms such as `[`, `「`, `『`, `【`; unrelated symbols such as `!` are absent; number keys, click/touch and PageUp/PageDown work |
| Punctuation-looking 注音 candidates | Compose a valid Chinese syllable that uses `.` or `,`, then press ↓ | All native Chinese candidates appear first, followed by related half-width/full-width/Chinese punctuation and then the raw-key recovery entry; selecting punctuation replaces only that character |

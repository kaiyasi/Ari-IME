...
2. **整段可修正**：送出前可以移動到任一字、重選詞語、插入或刪除文字，
   最後再一次送出；已送出的短中文選取範圍也能用 Fcitx5 原生周邊文字
   重新開啟候選修正，不必刪除重打。
3. **上下文選字**：完整音節會使用 libchewing 的詞語和上下文模型；
   完成音節後不自動彈出候選窗，按 Down 才進入互動選字，詞語候選優先
   於單字候選。
...
目前已提供可選的「Space 選字相容模式」給習慣傳統注音的使用者；不改變
目前混合模式的預設鍵義。
...
- 保持即時顯示的結果和候選窗第一候選一致。
- 完成中文音節後不自動顯示候選窗；使用者按 Down 後才進入互動選字。
- 候選窗在句尾開啟時仍保留包含游標字元的完整詞語候選。
- 選完詞後可以直接繼續輸入，Right 可以到達句尾插入點。
- 在支援 Fcitx5 surrounding text 的非敏感欄位，選取不超過 32 個純中文
  字元後按 `Ctrl+Alt+R` 可重新進入候選編輯；不符合安全條件時快捷鍵必須
  原樣交給應用程式。
...
- 使用者明確選過或匯入的詞要透過額外的 libchewing 學習次數適應習慣；實際
  候選排序交給語境模型，不在每個音節完成時偷偷開候選窗做 Ari 硬排序。
  普通接受結果仍只做一般學習，不能整批變成 Ari 的硬排序。
...
- 長 preedit 超過 libchewing active window 時，仍可編輯，但文件要明確說明
  詞語模型的上下文上限。
- 標點快捷鍵使用 Fcitx5 原生設定，可在 `Ctrl+Shift`、`Alt+Shift`、`Ctrl`、
  `Alt` 或停用之間選擇；中文頓號 `、` 的快捷鍵要有明確文件和回歸測試，
  不把避開瀏覽器／編輯器衝突的責任硬寫死在 Ari。
- 可選的傳統 Space 選字相容模式使用 Fcitx5 原生設定；預設仍維持 Ari 的
  混合輸入 Space＝一聲／空格與 Enter 送出契約。
- 已送出文字重新選字使用 Fcitx5 的 surrounding text 與原生候選窗，不新增
  Ari 工具列；只支援短純中文選取，避免在未知編輯器狀態下刪除使用者文字。
...
| 自動候選窗 | `hk4g4`、不按 Down | 不顯示候選窗；結果仍由 libchewing 語境模型排序，按 Down 才開啟候選 |
| 一聲 | `y`、Space | 產生有效漢字，`資` 可從候選取得 |
| 修正 | 句中游標、重選、Right 到句尾、再輸入 | 不需刪除整句即可完成 |
| 已送出文字修正 | 送出 `測試`、反白選取、`Ctrl+Alt+R`、選 `策士` | 原選取範圍改成 `策士`；Esc／焦點重置會恢復原文 |
